...
with tf.Session() as sess:
    init.run()
    restore_saver.restore(sess, "./tmp/my_model_final.ckpt") # restore the saved layers
...
my_dense_layer = partial(
    ...
    kernel_regularizer=tf.contrib.layers.l1_regularizer(scale))  # add regularizaton parameter here
...
with tf.name_scope("dnn"):
    hidden1 = tf.layers.dense(X_drop, n_hidden1, activation=tf.nn.relu,
                              name="hidden1")
    hidden1_drop = tf.layers.dropout(hidden1, dropout_rate, training=training)  ###ba
...
# In[102]: TO Read the book, to be read the book PP 307
...
if True:
    ...
    with tf.Session() as sess:                                              # not shown in the book
        ...
        save_path = saver.save(sess, "./tmp/my_model_final.ckpt")               # not shown
...
# In[118]:
n_epochs = min(1000, n_epochs_min)
batch_size = 20
...
class DNNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y, n_epochs=min(n_epochs_min, 100), X_valid=None, y_valid=None):
...
dnn_clf.fit(X_train1, y_train1, n_epochs=min(n_epochs_min, 1000), X_valid=X_valid1, y_valid=y_valid1)
...
rnd_search.fit(X_train1, y_train1, X_valid=X_valid1, y_valid=y_valid1, n_epochs=(n_epochs_min, 1000))
...
dnn_clf_bn.fit(X_train1, y_train1, n_epochs=min(n_epochs_min, 1000), X_valid=X_valid1, y_valid=y_valid1)
...
dnn_clf_dropout = DNNClassifier(activation=leaky_relu(alpha=0.1), batch_size=500, learning_rate=0.01,
                                n_neurons=90, random_state=42,
                                dropout_rate=0.5)
dnn_clf_dropout.fit(X_train1, y_train1, n_epochs=min(n_epochs_min, 1000), X_valid=X_valid1, y_valid=y_valid1)
...
restore_saver = tf.train.import_meta_graph("./tmp/my_best_mnist_model_0_to_4.meta")
...
with tf.Session() as sess:
    init.run()
    restore_saver.restore(sess, "./tmp/my_best_mnist_model_0_to_4")
    t0 = time.time()
    ...
    for epoch in range(n_epochs):
        ...
        if loss_val < best_loss:
            save_path = five_frozen_saver.save(sess, "./tmp/my_mnist_model_5_to_9_five_frozen")
            best_loss = loss_val
            checks_without_progress = 0
        else:
            checks_without_progress += 1
...
with tf.Session() as sess:
    five_frozen_saver.restore(sess, "./tmp/my_mnist_model_5_to_9_five_frozen")
...
with tf.Session() as sess:
    ...
    for epoch in range(n_epochs):
        ...
        loss_val, acc_val = sess.run([loss, accuracy], feed_dict={X: X_valid2, y: y_valid2})
        if loss_val < best_loss:
            save_path = four_frozen_saver.save(sess, "./tmp/my_mnist_model_5_to_9_four_frozen")
            best_loss = loss_val
            checks_without_progress = 0
        else:
            checks_without_progress += 1
...
with tf.Session() as sess:
    four_frozen_saver.restore(sess, "./tmp/my_mnist_model_5_to_9_four_frozen")
    acc_test = accuracy.eval(feed_dict={X: X_test2, y: y_test2})
    print("Final test accuracy: {:.2f}%".format(acc_test * 100))
...
with tf.Session() as sess:
    ...
    for epoch in range(n_epochs):
        ...
        loss_val, acc_val = sess.run([loss, accuracy], feed_dict={X: X_valid2, y: y_valid2})
        if loss_val < best_loss:
            save_path = two_frozen_saver.save(sess, "./tmp/my_mnist_model_5_to_9_two_frozen")
            best_loss = loss_val
            checks_without_progress = 0
        else:
            checks_without_progress += 1
...
with tf.Session() as sess:
    two_frozen_saver.restore(sess, "./tmp/my_mnist_model_5_to_9_two_frozen")
    acc_test = accuracy.eval(feed_dict={X: X_test2, y: y_test2})
    print("Final test accuracy: {:.2f}%".format(acc_test * 100))
...
with tf.Session() as sess:
    ...
    for epoch in range(n_epochs):
        ...
        loss_val, acc_val = sess.run([loss, accuracy], feed_dict={X: X_valid2, y: y_valid2})
        if loss_val < best_loss:
            save_path = no_frozen_saver.save(sess, "./tmp/my_mnist_model_5_to_9_no_frozen")
            best_loss = loss_val
            checks_without_progress = 0
        else:
            checks_without_progress += 1
...
with tf.Session() as sess:
    no_frozen_saver.restore(sess, "./tmp/my_mnist_model_5_to_9_no_frozen")
    acc_test = accuracy.eval(feed_dict={X: X_test2, y: y_test2})
    print("Final test accuracy: {:.2f}%".format(acc_test * 100))
...
dnn_clf_5_to_9.fit(X_train2, y_train2, n_epochs=min(n_epochs_min, 1000), X_valid=X_valid2, y_valid=y_valid2)
...
with tf.Session() as sess:
    ...
    save_path = saver.save(sess, "./tmp/my_digit_comparison_model.ckpt")
...
with tf.Session() as sess:
    init.run()
    restore_saver.restore(sess, "./tmp/my_digit_comparison_model.ckpt")
    ...
    save_path = saver.save(sess, "./tmp/my_mnist_model_final.ckpt")
