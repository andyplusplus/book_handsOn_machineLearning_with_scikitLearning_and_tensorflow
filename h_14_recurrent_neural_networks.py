...
Wy = tf.Variable(tf.random_normal(shape=[n_neurons,n_neurons],dtype=tf.float32))    # (3, 5)
b = tf.Variable(tf.zeros([1, n_neurons], dtype=tf.float32))             # (1, 5)

Y0 = tf.tanh(tf.matmul(X0, Wx) + b)         # (4, 5)
...
X0 = tf.placeholder(tf.float32, [None, n_inputs])  # (4, 5)
...
def show_graph(t0 = None, t1 = None, t2 = None, t3 = None):
    pass
...
X = tf.placeholder(tf.float32, [None, n_steps, n_inputs]) # (mini-batch size,
...
# ## Setting the sequence lengths  pp389
...
# ## Training a sequence classifier  pp 389
...
mnist = get_mnist_gzs() # TOBEHERE
X_test = mnist.test.images.reshape((-1, n_steps, n_inputs))
y_test = mnist.test.labels

# In[36]: 391
n_epochs = min(n_epochs_min, 100)
batch_size = 150
...
# In[51]:
n_iterations = min(1500, n_iterations_min)
batch_size = 50
...
with tf.Session() as sess:
    ...
    saver.save(sess, "./tmp/my_time_series_model") # not shown in the book

# In[52]:
with tf.Session() as sess:                          # not shown in the book
    saver.restore(sess, "./tmp/my_time_series_model")   # not shown
...
# In[60]:
n_iterations = min(n_iterations_min, 1500)
batch_size = 50
...
with tf.Session() as sess:
    ...
    saver.save(sess, "./tmp/my_dropout_time_series_model")

# In[83]:
with tf.Session() as sess:
    saver.restore(sess, "./tmp/my_dropout_time_series_model")
...
# In[109]:
num_steps = min(10001, n_steps_min)
...
with tf.Session() as sess:
    ...
    for epoch in range(n_epochs):
        ...
        saver.save(sess, "./tmp/my_reber_classifier")
...
with tf.Session() as sess:
    saver.restore(sess, "./tmp/my_reber_classifier")
