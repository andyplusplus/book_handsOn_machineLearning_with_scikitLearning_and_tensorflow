...
if False:
    def load_data():
        # path = 'data/mnist_2.npz' #此处的path为你刚刚防止mnist.py的目录。注意斜杠
        path = 'data/mnist.npz' #此处的path为你刚刚防止mnist.py的目录。注意斜杠
...
if False:
    ...
    input_fn = tf.estimator.inputs.numpy_input_fn(
        x={"X": X_train}, y=y_train, num_epochs=min(40, num_epoches_4_test) , batch_size=50, shuffle=True)
...
#     rather than `tf.layers.dense()` (which did not exist when this chapter was written).
#     It is now preferable to use `tf.layers.dense()`, because anything in the contrib module
#     may change or be deleted without notice. The `dense()` function is almost identical to the `fully_connected()` function, except for a few minor differences:
