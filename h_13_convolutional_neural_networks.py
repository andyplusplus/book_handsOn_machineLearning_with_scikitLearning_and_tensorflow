...
if True:
    ...
    # In[5]:
    fmap = np.zeros(shape=(7, 7, 1, 2), dtype=np.float32)    # filter_height, filter_width, in_channels, out_channels
    fmap[:, 3, 0, 0] = 1
    ...
    with tf.Session() as sess:
        output = convolution.eval(feed_dict={X: images}) # output is a tensor, have same type as input
    ...
    # In[19]:
    X = tf.placeholder(tf.float32, shape=(None, height, width, channels))
    max_pool = tf.nn.max_pool(X, ksize=[1,2,2,1], strides=[1,2,2,1],padding="VALID")   # ksize  [batch size, height, width, channels
...
with tf.Session() as sess:
    ...
    for epoch in range(n_epochs):
        ...
        save_path = saver.save(sess, "./tmp/my_mnist_model")
...
# In[27]:
n_epochs = min(n_epochs_min,1000)
batch_size = 50
...
def fetch_pretrained_inception_v3(url=INCEPTION_V3_URL, path=INCEPTION_PATH):
    ...
    urllib.request.urlretrieve(url, tgz_path, reporthook=download_progress)  # Error here
...
# In[32]:
fetch_pretrained_inception_v3() # TOBEHERE
...
with tf.Session() as sess:
    ...
    for epoch in range(n_epochs):
        ...
        save_path = saver.save(sess, "./tmp/my_flowers_model")
...
with tf.Session() as sess:
    saver.restore(sess, "./tmp/my_flowers_model")
